...
void beb(int attempts, int time_slot_len) {
  ...
  int backoff_time =
      ...
      generate_random_integer(
          0, ((int)pow(2, attempts) - 1)); // Generate random backoff time
...
void request_words_binary_exponential_backoff(int client_sock_fd,
...
void request_words_sensing_and_beb(int client_sock_fd,
  ...
  while (true) {
    ...
    if (strcmp(sense_buffer, "BUSY\n") == 0) {
      std::cout << client_sock_fd << " : Server status is busy, waiting for "
                << client_info->time_slot_len << "ms" << std::endl;
      // beb(1, client_info->time_slot_len);
      sleep(client_info->time_slot_len / 1000);
      continue;
    } else if (strcmp(sense_buffer, "IDLE\n") == 0) {
      std::cout << client_sock_fd << " : Server status is idle, sending request"
                << std::endl;
      std::string req_payload = std::to_string(client_info->offset) + "\n";
      ...
      while (word_count < num_word_per_request) {
        ...
        std::cout << "Current buffer_tmp " << buffer_temp[0] << std::endl;
...
void *client_thread_slotted_aloha(void *arg) // Client thread function
{
  ...
  request_words_slotted_aloha(client_sock_fd,
